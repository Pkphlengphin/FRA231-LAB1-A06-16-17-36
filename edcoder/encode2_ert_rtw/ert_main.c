...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) encode2_M->Timing.taskTime0;
  encode2_step();
...
int main(int argc, char **argv)
{
  ...
  errorCode = extmodeInit(encode2_M->extModeInfo, &rtmGetTFinal(encode2_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(encode2_M, true);
    }
  }

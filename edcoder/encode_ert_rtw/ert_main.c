...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) encode_M->Timing.taskTime0;
  encode_step();
...
int main(int argc, char **argv)
{
  ...
  errorCode = extmodeInit(encode_M->extModeInfo, &rtmGetTFinal(encode_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(encode_M, true);
    }
  }
